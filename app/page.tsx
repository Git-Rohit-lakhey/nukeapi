...
const FEATURES = [
  ["⚡", "Parallel execution", "All integrations run in parallel — one failure never hides another result."],
  ["📄", "Signed PDF audit trail", "HMAC-SHA256 over the canonical result, stored on the row and embedded in the PDF."],
  ["🔁", "Retry with backoff", "10-second timeout with automatic retries on transient errors; client errors are never retried, and a slow provider cannot stall your request."],
  ["🔑", "Honest API keys", "API keys are hashed and never stored in readable form. Each key is shown only once, at creation."],
  ["🛡️", "Zero plaintext creds", "AES-256-GCM encryption at rest. The browser never sees the key, and the database never stores plaintext."],
  ["📊", "Usage that adds up", "Atomic usage metering — concurrent deletions cannot undercount your quota."],
] as const;

const FAQS = [
  ["How fast can I go live?", "About 15 minutes: sign up, connect one integration, create a key, send your first POST. The docs page has copy-paste examples in curl, Node, Python, and five more languages."],
  ...
  ["How does billing work?", "Checkout is via Dodo Payments. The webhook upgrades your plan automatically. Cancel anytime from settings — access continues until the end of the billing period."],
  ["Is my data encrypted?", "Yes. Connector credentials are encrypted with AES-256-GCM before storage. API keys are bcrypt hashes. Audit rows are HMAC-signed."],
] as const;

export default function Home() {
  const codeExample = `curl -X POST https://www.nukeapi.dev/api/v1/delete-user \\
    ...
    <div style={{ background: "#0a0a0c", color: "#d8d8d8", fontFamily: "'SF Mono','Fira Code',monospace", minHeight: "100vh" }}>
        ...
        <div style={{ maxWidth: 1080, margin: "0 auto", width: "100%", display: "grid", gridTemplateColumns: "1.1fr .9fr", gap: 56, alignItems: "center", position: "relative" }} className="g2">
          <div className="anim-fadeUp">
            ...
            <p style={{ fontSize: 15.5, color: "#5c5c68", maxWidth: 480, lineHeight: 1.85, marginBottom: 32 }}>
              NukeAPI deletes a user from Stripe, Mailchimp, HubSpot, Intercom, and your database in parallel — and provides a signed PDF audit trail. Live in 15 minutes.
            </p>
            ...
            <p style={{ fontSize: 12, color: "#3a3a44", marginBottom: 32 }}>Free Sandbox · 20 deletions/month · No credit card required</p>
            <div style={{ display: "flex", gap: 36, flexWrap: "wrap", borderTop: "1px solid #15151a", paddingTop: 24 }}>
                {[["15min", "to first deletion"], ["78", "integrations"], ["207/429", "honest statuses"]].map(([v, l]) => (
      ...
      <section id="pricing" style={{ padding: "88px 6%", background: "#080809", borderTop: "1px solid #121218" }}>
        <div style={{ maxWidth: 1080, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 36 }}>
            ...
            <p style={{ fontSize: 14, color: "#55555f" }}>No seats. No hidden fees. Cancel anytime. Secure checkout via Dodo Payments.</p>
          </div>
      ...
      <section style={{ padding: "88px 6%", borderTop: "1px solid #121218" }}>
        <div style={{ maxWidth: 1080, margin: "0 auto", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 48, alignItems: "start" }} className="g2">
          <div>
            ...
            <p style={{ fontSize: 14, color: "#55555f", lineHeight: 1.85 }}>GDPR gives you 30 days, CCPA 45. Miss it and fines reach up to €20M or 4% of global revenue — whichever is higher. Every deletion produces a signed PDF your lawyers can file.</p>
      ...
      <section style={{ padding: "88px 6%", background: "#080809", borderTop: "1px solid #121218" }}>
        <div style={{ maxWidth: 1080, margin: "0 auto" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 16 }} className="g3">
            <div className="card card-hover" style={{ padding: 24 }}>
              ...
              <div style={{ fontSize: 13, color: "#5c5c68", lineHeight: 1.8 }}>
                ...
                A manual runbook across 5+ dashboards rarely finishes in time — and &quot;we couldn&apos;t find them&quot; is not a defense.
              </div>
            </div>
            <div className="card card-hover" style={{ padding: 24 }}>
              <div style={{ fontSize: 22, marginBottom: 12 }}>⚖️</div>
            ...
            <div className="card card-hover" style={{ padding: 24 }}>
              ...
              <div style={{ fontSize: 13, color: "#5c5c68", lineHeight: 1.8 }}>
                Connect each system once. Afterwards one API call fans out in parallel with timeout and retry,
                and returns an HMAC-signed PDF your lawyers can file. No runbooks, no manual dashboard work, no spreadsheet tracking.
              </div>
            </div>
          </div>
          ...
          <p style={{ fontSize: 13, color: "#3d3d48", marginTop: 20, textAlign: "center" }}>
            Developer-friendly by design: copy-paste curl, Node and Python examples in <Link href="/docs" style={{ color: LIME }}>the docs</Link> — most teams go live the same day.
          </p>
        </div>
      </section>
      ...
      <section style={{ padding: "88px 6%", borderTop: "1px solid #121218" }}>
        <div style={{ maxWidth: 1080, margin: "0 auto", background: `linear-gradient(180deg, #0e1500, #0a0d00)`, border: `1px solid ${LIME}30`, borderRadius: 20, padding: "56px 48px", textAlign: "center", boxShadow: "0 30px 80px rgba(200,241,53,.08)" }}>
          <h2 style={{ fontSize: "clamp(1.6rem,3.4vw,2.6rem)", fontWeight: 900, letterSpacing: "-.03em", color: "#fff", marginBottom: 12 }}>Your next data-subject request could arrive at any time.</h2>
