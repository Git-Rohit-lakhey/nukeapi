...
export const HTTP_BATCH_3: ConnectorSpec[] = [
  ...
  // ── Braintree: official SDK, not a simple REST endpoint — CustomSpec ──
  {
    ...
    run: async (email, creds) => {
      ...
      try {
        // Official SDK has no ergonomic types for this flow; the calls below
        // are runtime-verified (search.each / customer.delete → {success}).
        const braintree = (await import("braintree")) as any;
