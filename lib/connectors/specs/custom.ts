...
/**
 * Driver-based (non-HTTP) CustomSpec closures.
 *
 * Each `run` below is the EXACT logic of the original connector function,
 * dispatchable uniformly by the universal engine. Behavior, endpoints, status
 * handling, count logic, and messages are preserved exactly — only the typed
 * credential interface was replaced with string-indexed `creds.fieldName`
 * access and the dynamic SDK imports were kept inside `run` (so SDKs load
 * only when that connector actually executes, never into a client bundle).
 */
